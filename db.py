...
class DB:
    ...
    def AddToCart(prod, username):
        cursor.execute(f"INSERT INTO koszyk (`username`, `produkt`) VALUES ('{username}', '{prod}');")
        connection.commit()

    def RemoveFromCart(prod, username):
        cursor.execute(f"SELECT _id FROM `koszyk` WHERE (`username` = '{username}' AND `produkt` = '{prod}');")
        fetchedRows = cursor.fetchall()

        for x in fetchedRows:
            connection.cursor().execute( f"DELETE FROM `koszyk` WHERE (`_id` = '{x[0]}');")
            connection.commit()
    ...
    def FromCartToOrder(cart,login):
        for x in cart:
            try:
                cursor.execute(f"SELECT ilosc FROM produkty WHERE _id = '{x.produkt.id}';")
                fetchedRow = cursor.fetchone()
                connection.cursor().execute(f"UPDATE `produkty` SET `ilosc` = '{fetchedRow[0]-x.ilosc}' WHERE (`_id` = '{x.produkt.id}');")

                cursor.execute(f"SELECT _id FROM `koszyk` WHERE (`username` = '{login}' AND `produkt` = '{x.produkt.name}');")
                fetchedRows = cursor.fetchall()
                for x in fetchedRows:
                    connection.cursor().execute(f"DELETE FROM `koszyk` WHERE (`_id` = '{x[0]}');")
            finally:
    ...
    def loadOrders(login):
        cursor.execute(f"SELECT wartosc, utworzenie, stan FROM zamowienia WHERE username = '{login}';")
